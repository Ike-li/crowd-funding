...
# 导入 GitHub 图床工具
from src.apps.extensions.upload_images_to_github_drawing_bed import (
...
def get_current_user():
    ...
    if user_name is None:
        ...
        user_account_query_cql = (
            "SELECT user_account " "FROM users.user " "WHERE user_name = %s;"
        )
        account_rows = cass_session.execute(
            user_account_query_cql, user_account_query_list
        )
...
def user_register():
    ...
    if len(cass_session.execute(cql, ls).all()) != 0:
        ...
        len(
            (
                cass_session.execute(
                    "SELECT user_phone " "FROM users.user " "WHERE user_phone = %s;",
                    ls1,
                )
            ).all()
        )
        ...
        len(
            (
                cass_session.execute(
                    "SELECT user_email " "FROM users.user " "WHERE user_email = %s;",
                    ls2,
                )
            ).all()
        )
...
def user_add_function():
    ...
    if request.method == "POST":
        ...
        upload_path = os.path.join(
            base_path, "../../static/user_images", secure_filename(new_file_name)
        )  # 重命名图片名
    ...
    cql = (
        "INSERT INTO functions.functions_request "
    ...
    cql1 = (
        "INSERT  INTO functions.functions_request_by_status "
    ...
    cql2 = (
        "INSERT INTO users.user_by_publish "
...
def user_functions(state):
    ...
    if state == "已通过":
        return render_template(
            "user/user_functions_published.html", args=functions, state=state
        )
    elif state == "审核中":
        return render_template(
            "user/user_functions_under_review.html", args=functions, state=state
        )
    elif state == "未通过":
        return render_template(
            "user/user_functions_failed.html", args=functions, state=state
        )
    elif state == "未审核":
        return render_template(
            "user/user_functions_not_reviewed.html", args=functions, state=state
        )
    else:
...
def function_published_one(function_id):
    ...
    function_published_cql = (
        "SELECT * " "FROM functions.functions " "WHERE function_id = %s;"
    )
    function_published_data = cass_session.execute(
        function_published_cql, function_published_list
    )
    function_published_details = function_published_data.all()
    return render_template(
        "user/user_function_published_details.html", args=function_published_details
    )
...
def function_unpublished_one(function_id):
    ...
    function_unpublished_cql = (
        "SELECT * " "FROM functions.functions_request " "WHERE function_id = %s;"
    )
    function_unpublished_data = cass_session.execute(
        function_unpublished_cql, function_unpublished_id
    )
    function_unpublished_details = function_unpublished_data.all()
    if function_unpublished_details[0].get("state") == "未审核":
        return render_template(
            "user/user_function_not_reviewed_details.html",
            args=function_unpublished_details,
        )
    return render_template(
        "user/user_function_unpublished_details.html", args=function_unpublished_details
    )
...
def my_donations():
    ...
    # 用户对该 FR 的打赏
    user_donate_cql = (
        "SELECT * " "FROM users.user_by_contribution " "WHERE user_name = %s;"
    )
...
def search_type(function_type):
    ...
    function_type_cql = (
        "SELECT * " "FROM functions.functions " "WHERE function_type = %s;"
    )
    functions_rows = cass_session.execute(function_type_cql, function_type_list)
    functions = functions_rows.all()
    return render_template(
        "index.html", functions=functions, function_type=function_type
    )
...
def search_time():
    ...
    function_type_cql = (
        "SELECT * " "FROM functions.functions " "WHERE created_at_date = %s;"
    )
    functions_rows = cass_session.execute(function_type_cql, function_type_list)
    functions = functions_rows.all()
...
def collect_function(function_id_str):
    ...
    function_query_cql = (
        "SELECT function_type,function_title,created_at,state "
        "FROM functions.functions "
        "WHERE function_id = %s;"
    )
    ...
    functions_collections_counter_cql = (
        "UPDATE functions.functions_collections_counter "
        "SET collections = collections +1 "
        "WHERE function_id = %s AND function_title = %s;"
    )
    cass_session.execute(
        functions_collections_counter_cql, functions_collections_counter_list
    )

    # 更新表 functions.functions_collections 的记录
    functions_collections_list = [
        function_id_uuid,
        collection_by_user,
        created_at,
        function_title,
    ...
    functions_collections_cql = (
        "INSERT INTO functions.functions_collections (function_id, collection_by_user, "
    ...
    user_by_collection_cql = (
        "INSERT INTO users.user_by_collection (user_name, function_id, function_title, "
        "function_type, state) "
        "VALUES (%s, %s, %s, %s, %s);"
    )
...
def function_publisher(function_id_str):
    ...
    function_published_query_cql = (
        "SELECT * " "FROM functions.functions " "WHERE function_id = %s;"
    )
    function_rows = cass_session.execute(
        function_published_query_cql, function_published_query_list
    )
...
def edit_function_not_reviewed():
    ...
    function_request_cql = (
        "UPDATE functions.functions_request "
    ...
    # 添加 FR 到 functions_request_by_status
    function_request_by_status_list = [
        created_at,
        function_title,
        function_type,
        publisher,
    ...
    function_request_by_status_cql = (
        "UPDATE functions.functions_request_by_status "
        "SET created_at = %s, function_title = %s, function_type = %s, publisher = %s "
        "WHERE state = %s "
        "AND function_id = %s;"
    )
    cass_session.execute(
        function_request_by_status_cql, function_request_by_status_list
    )

    # 添加任务到 users.user_by_publish, 先删再插入
    user_by_publish_delete_list = [publisher, function_id]
    user_by_publish_delete_cql = (
        "DELETE "
        "FROM users.user_by_publish "
        "WHERE user_name = %s "
        "AND function_id = %s;"
...
def delete_function_not_reviewed():
    ...
    functions_request_delete_cql = (
        "DELETE " "FROM functions.functions_request " "WHERE function_id = %s;"
    )
    ...
    functions_request_by_status_delete_cql = (
        "DELETE "
    ...
    cass_session.execute(
        functions_request_by_status_delete_cql, functions_request_by_status_delete_list
    )
...
def donate_function():
    ...
    donate_money_query_cql = (
        "SELECT * " "FROM functions.functions " "WHERE function_id = %s;"
    )
    donated_function_rows = cass_session.execute(
        donate_money_query_cql, donate_money_query_list
    )
    ...
    user_by_contribution_insert_list = [
        session["user_name"],
        function_id_uuid,
        created_at,
        donated_function.get("function_title"),
        donated_function.get("function_type"),
        donate_money,
        donated_function.get("state"),
    ]
    user_by_contribution_insert_cql = (
        "INSERT INTO users.user_by_contribution "
        "(user_name, function_id, create_at, function_title, function_type, money, "
        "state) "
        "VALUES (%s, %s, %s, %s, %s, %s, %s);"
    )
    cass_session.execute(
        user_by_contribution_insert_cql, user_by_contribution_insert_list
    )

    # 插入到 functions.functions_contribution 表
    functions_contribution_insert_list = [
        function_id_uuid,
        created_at,
        session["user_name"],
        donated_function.get("function_title"),
    ...
    functions_contribution_insert_cql = (
        "INSERT INTO functions.functions_contribution "
        "(function_id, created_at, contribution_by_user, function_title, "
        "function_type, money) "
        "VALUES (%s, %s, %s, %s, %s, %s);"
    )
    cass_session.execute(
        functions_contribution_insert_cql, functions_contribution_insert_list
    )

    # 用户账户减去相应金额
    user_name = session["user_name"]
    user_money_query_list = [user_name]
    user_money_query_cql = (
        "SELECT user_account " "FROM users.user " "WHERE user_name = %s;"
    )
    ...
    user_money_insert_cql = (
        "UPDATE users.user " "SET user_account = %s " "WHERE user_name = %s;"
    )
