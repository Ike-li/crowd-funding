...
@load_admin
def admin_center():
    functions_request_by_status_cql = (
        "SELECT * " "FROM functions.functions_request_by_status " "WHERE state = '未审核';"
    )
...
def admin_login():
    ...
    admin_name_cql = "SELECT * " "FROM functions.admin " "WHERE admin_name = %s;"
    ...
    # 如果管理员存在则对比密码是否相等
    admin_password_cql = (
        "SELECT admin_passwd " "FROM functions.admin " "WHERE admin_name = %s;"
    )
...
def functions_state():
    ...
    if state == "未审核":
        return render_template(
            "admin/admin_functions_not_reviewed.html", args=functions, state=state
        )
    elif state == "审核中":
        return render_template(
            "admin/admin_functions_under_review.html", args=functions, state=state
        )
    elif state == "未通过":
        return render_template(
            "admin/admin_functions_under_review.html", args=functions, state=state
...
def check_function():
    ...
    cql1 = (
        "DELETE FROM functions.functions_request_by_status "
        "WHERE state = %s "
        "AND function_id = %s;"
    )
    ...
    ls2 = [function_id]
    cql2 = (
        "SELECT function_type,function_title,publisher, created_at "
    ...
    ls4 = [
        state,
        function_id,
        ls3[0].get("created_at"),
        ls3[0].get("function_title"),
        ls3[0].get("function_type"),
        ls3[0].get("publisher"),
    ]
    cql4 = (
    ...
    if state == "已通过":
        ...
        ls5 = [function_id]
        cql5 = "SELECT * " "FROM functions.functions_request " "WHERE function_id = %s;"
        ...
        ls6 = [
            ...
            created_at_date,
            0,
            function_by_request["crowd_funding_days"],
            function_by_request["crowd_funding_money"],
            function_by_request["function_content"],
            function_by_request["function_cover"],
            function_by_request["function_introduction"],
            function_by_request["function_title"],
            function_by_request["function_type"],
            function_by_request["publisher"],
            function_by_request["state"],
        ]
        cql6 = (
        ...
        ls7 = [
            function_by_request["function_type"],
            function_by_request["function_id"],
            function_by_request["crowd_funding_money"],
            function_by_request["function_cover"],
            function_by_request["function_introduction"],
            function_by_request["function_title"],
        ...
        cql7 = (
            "INSERT INTO functions.functions_by_type "
        ...
        cql8 = (
            "INSERT INTO functions.functions_by_time "
        ...
        cql13 = (
            "DELETE FROM users.user_by_publish"
            " WHERE user_name = %s "
            "AND function_id = %s;"
        )
...
def show_all_collections():
    all_functions_collections_cql = (
        "SELECT function_id,function_title,collections "
        "FROM functions.functions_collections_counter;"
    )
    all_functions_collections__rows = cass_session.execute(
        all_functions_collections_cql
    )
    collection = all_functions_collections__rows.all()
    return render_template(
        "admin/admin_all_functions_collections.html", collectiion=collection
    )
...
def show_one_collections(function_id):
    ...
    return render_template(
        "admin/admin_one_functions_collections.html", collections=collections
    )
...
def show_crowd_funding_success_functions():
    success_functions_cql = "SELECT * " "FROM functions.success_functions;"
    success_functions_rows = cass_session.execute(success_functions_cql)
    success_functions = success_functions_rows.all()
    return render_template(
        "admin/admin_success_functions.html", success_functions=success_functions
    )
...
def show_crowd_funding_fail_functions():
    fail_functions_cql = "SELECT * " "FROM functions.fail_functions;"
    fail_functions_rows = cass_session.execute(fail_functions_cql)
    fail_functions = fail_functions_rows.all()
    return render_template(
        "admin/admin_fail_functions.html", fail_functions=fail_functions
    )
...
def all_functions_donations():
    all_functions_donations_cql = "SELECT * " "FROM functions.functions_contribution;"
    ...
    return render_template(
        "admin/admin_all_functions_donations.html",
