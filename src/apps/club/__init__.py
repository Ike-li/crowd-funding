...
def create_app():
    ...
    def refund(function_id):
        ...
        refund_functions_query_cql = (
            "SELECT contribution_by_user, money "
            "FROM functions.functions_contribution "
            "WHERE function_id = %s;"
        )
        money_list_rows = cass_session.execute(
            refund_functions_query_cql, refund_functions_query_list
        )
        ...
        for money_dict in money_list:
            ...
            user_account_by_query = (
                cass_session.execute(user_account_query_cql, user_account_query_list)
            ).all()
            user_account_insert_list = [
                user_name,
                user_account_by_query[0].get("user_account") + money,
            ]
            user_account_insert_cql = (
                "INSERT INTO users.user" "(user_name,user_account) " "VALUES (%s, %s);"
            )
    ...
    def index():
        ...
        if function_id is None:
            functions_cql = "SELECT * " "FROM functions.functions " "limit 9;"
            ...
            for function in functions:
                ...
                if function.get("closing_time") < (datetime.now()):
                    # 检查任务是否在到期的前提下达到筹集金额
                    if function.get("crowd_funding_current_money") >= function.get(
                        "crowd_funding_money"
                    ):
                        # 任务众筹成功，插入到 functions.success_functions 表
                        success_function_insert_list = [
                            function.get("function_id"),
                            function.get("closing_time"),
                            function.get("created_at"),
                            function.get("created_at_date"),
                        ...
                        success_function_insert_cql = (
                            "INSERT INTO functions.success_functions "
                        ...
                        cass_session.execute(
                            success_function_insert_cql, success_function_insert_list
                        )
                        ...
                        publisher_account_rows = cass_session.execute(
                            publisher_query_cql, publisher_query_list
                        )
                        publisher_account = ((publisher_account_rows.all())[0]).get(
                            "user_account"
                        )
                        publisher_reward_list = [
                            publisher,
                            publisher_account + int(reward_money * 0.1),
                        ]
                        publisher_reward_cql = (
                            "INSERT INTO users.user(user_name, user_account) "
                            "VALUES (%s, %s);"
                        )
                        cass_session.execute(
                            publisher_reward_cql, publisher_reward_list
                        )
                        ...
                        fail_function_insert_cql = (
                            "INSERT INTO functions.fail_functions "
                            "(function_id, closing_time, created_at, created_at_date, "
                            "crowd_funding_days, crowd_funding_money, function_content, "
                            "function_cover, function_introduction, function_title, "
                            "function_type, publisher) "
                        ...
                        cass_session.execute(
                            fail_function_insert_cql, fail_function_insert_list
                        )
                        # 退款
                        refund(function.get("function_id"))
                        # 删除 functions.functions_contribution 里的记录
                        delete_functions_contribution_list = [
                            function.get("function_id")
                        ]
                        delete_functions_contribution_cql = (
                            "DELETE FROM functions.functions_contribution "
                            "WHERE function_id = %s;"
                        )
                        cass_session.execute(
                            delete_functions_contribution_cql,
            ...
            functions_page_rows = cass_session.execute(
                functions_page_cql, functions_page_list
            )
            ...
            for function in functions_page:
                ...
                if function.get("closing_time") < (datetime.now()):
                    ...
                    if function.get("crowd_funding_current_money") >= function.get(
                        ...
                        delete_function_list = [function.get("function_id")]
                        delete_function_cql = (
                            "DELETE FROM " "functions.functions WHERE function_id = %s;"
                        )
                        cass_session.execute(delete_function_cql, delete_function_list)
                    else:
                        # 插入到 functions.fail_functions 表
